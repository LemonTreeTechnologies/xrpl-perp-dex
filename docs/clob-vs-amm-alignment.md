...
**Status:** **RESOLVED in favor of Variant A on 2026-04-13.** Tom confirmed directly: *"(A) a vault that uses a curve-based algorithm to compute bid/ask prices and then posts those as limit orders on the existing CLOB (CLOB is still the execution layer, the pool is a smarter market maker)."* The CLOB stays. `vault_mm` gets a smarter pricing algorithm. No enclave changes. All dependent documents (`comparison-arch-network.md`, `btc-perp-dex-feasibility.md`, `sgx-enclave-capabilities-and-limits.md`) remain accurate as-written. This document is kept as a historical reference for why the post-hackathon direction was interpreted this way and not the other way; future contributors can read §2 and §3 to understand what the two possibilities were and why the team explicitly picked the CLOB-preserving one. Bilingual: Russian counterpart at `clob-vs-amm-alignment-ru.md`.
