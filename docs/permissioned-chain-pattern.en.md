...
- **TEE-attested business logic** — the perpetuals matching engine, vault, settlement math, and threshold settlement signing (XRPL SignerList M-of-N ECDSA — the enclave also carries FROST/Schnorr primitives, but they sign nothing on XRPL). Runs in SGX today; portable to TDX / SEV-SNP / Nitro in principle, see `EthSignerEnclave/docs/contingency/` in the private repo for the substrate-migration analyses.
