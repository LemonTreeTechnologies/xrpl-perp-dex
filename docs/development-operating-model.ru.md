...
- **Mode S — Software sync.** Update enclave + orchestrator binaries, MRENCLAVE bump, fresh DKG. **НЕ** трогает escrow funds, master-key state, или `SignerListSet` membership. Может запускаться на пустом escrow, на partially-funded escrow, или на fully-provisioned escrow — процедура идентична и indifferent.
