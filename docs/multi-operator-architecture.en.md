...
2. **No single operator can produce a valid FROST signature.** FROST 2-of-N (or higher) prevents it; `frost_group` state is sealed inside each enclave. This is a **separate mechanism from #1 and is not what protects XRPL custody** — no perp-dex product path signs with FROST today; the FROST group exists for the planned Bitcoin/Taproot leg, and the FROST group key is not on the XRPL SignerList.
...
6. **The signing-policy check is orchestrator-anchored — Pattern 318 deviation acknowledged.** The orchestrator-anchored signing-policy check (X-C1 hardening per audit Round 4-6) lives in code that the operator host controls and SGX does not attest. The compensating control is the loopback constraint — three layers: enclave `bind_address` self-check (perp REQ-6 C3), orchestrator `ensure_loopback_url` on `local_signer.enclave_url` (perp REQ-6 C2), OS firewall ACLs (out-of-band, see `feedback_firewall_topology_neutrality.md`). Any future change putting `/pool/sign` non-loopback or any new signing path bypassing `validate_signing_policy` invalidates this equivalence and requires a fresh policy-anchor analysis (Pattern 318 deviation per audit playbook). The structurally-correct enclave-anchored alternative (`ecall_check_pending_signing`) is tracked as RESP-5 §C5 follow-up, scheduled post-mainnet-thaw.
7. **Production-mode is unreachable without reproducible MRENCLAVE produced independently by ≥N operators.** Foundation invariant established 2026-05-03 (peer of #5). If only one operator can produce a given MRENCLAVE, the multi-operator trust model degenerates to "trust one operator's build environment" — supply-chain compromise of that one host equals compromise of the entire group. Build does not require SGX hardware; the prerequisite is a committed reproducible Dockerfile (`EthSignerEnclave/Dockerfile.azure`) with pinned toolchain versions, a GHA build-gate that publishes MRENCLAVE per commit, and an N-of-M cross-check procedure where ≥N independent operators rebuild and publish bit-identical MRENCLAVE values that are then registered on the on-chain MRENCLAVE allowlist (per REQ-7). The current sandbox modes (§1.1, §1.2 of `docs/development-operating-model.md`) tolerate single-operator builds because they hold no real customer state; production-mode (§1.3) is gated on this rule AND on #5. See `docs/build-requirements.{en,ru}.md` for the layered Build / Sign / Run policy and `feedback_reproducible_build_foundation.md` for the rule.
...
### 11.2 Retired SSH-driven debt

The following SSH-driven modules existed during the Phase 2.1c transition. They violated the multi-operator model (each baked in a cross-operator SSH assumption that production cannot honor). They have been retired.

- ~~`orchestrator/src/dkg_bootstrap.rs`~~ (commits `5fe5aa1`+`78710a9` + retired in 2.1c-F): SSH-driven DKG ceremony. Replaced by `dkg-coordinate` (cluster-coordinated, libp2p-driven).
- ~~`orchestrator/src/cluster_deploy.rs`~~ (commit `fefd3c9` + retired in 2.1c-F): SSH-driven multi-node binary swap + service lifecycle. Replaced by `node-deploy` (node-local). Coordinated MRENCLAVE bumps are governance, not a single command.

### 11.3 Replacement plan — completed

| Phase | Subcommand | Class | Replaces | Status |
|---|---|---|---|---|
