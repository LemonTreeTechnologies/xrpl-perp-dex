...
без multi-op signing  │   multi-op XRPL 2-of-3   │
...
2. **Deploy на Azure testnet-cluster (staging).** Binary копируется на все 3 ноды, systemd swap. Cluster-only поведение валидируется здесь: sequencer failover, multi-operator подпись XRPL-мультиподписью, FROST DKG / перенос долей, real DCAP attestation, libp2p mesh динамика. Production-shape поведение появляется здесь — Hetzner by design не покажет.
...
| Multi-operator подпись (XRPL-мультиподпись + FROST DKG) | Azure mandatory |
