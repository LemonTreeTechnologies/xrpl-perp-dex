...
>
> **Это НЕ доказательство платёжеспособности (AC-E1-6).** Пока не внедрены
> **AC-R1-5b** (сохранение PnL-контрагента) и **AC-F8-4** (изолированный perp-only
> reset), опубликованный `(epoch, root, snapshot)` НЕЛЬЗЯ представлять — ни в
> презентации, ни в доках, ни on-chain-потребителю — как доказательство
> платёжеспособности книги. Он подтверждает *кто подписал* и *структуру
> обязательств*, а не что активы их покрывают. Безопасно fail только направление
> «обязательства завышены» (`custody_ok` ложно-отказывает → нет публикации);
> занижение могло бы опубликовать корень, который *выглядит* платёжеспособным, но
> нет — именно это закрывает AC-R1-5b. On-chain контракт назван `ReservesRegistry`
> по причинам деплоя; имя — не заявление.
>
> **Исключение operator-capital ДЕКЛАРИРУЕТСЯ, не доказывается (reserves-input attestation).**
> v2-`snapshot` коммитит канонический хэш operator-ДЕКЛАРИРОВАННОГО набора excluded-senders
> (чьи платежи в escrow — это custody, не liabilities). Это **tamper-EVIDENT** — исключение
> закоммичено on-chain, неизменяемо-постфактум и атрибутируемо, поэтому неверно-исключённый
> депозит юзера или тихая смена политики **обнаружимы** пострадавшим юзером или любым
...
> которые охватил кворум, → claim «N независимых наблюдений» аудируем из маркера. Это
> аттестует **разнообразие источников** (другое свойство, чем исключение выше) и тоже
> **host-ДЕКЛАРИРОВАНО, не доказано** — различность enforced на момент церемонии, но
> записанные fingerprints — это то, что сообщает orch, а не in-enclave доказательство.
> Observation-кворум — **не** backing; **AC-BASE-2″ SPV остаётся mainnet-гейтом.**
