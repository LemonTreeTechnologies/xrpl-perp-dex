...
- **Mode S — Software sync.** Update enclave + orchestrator binaries, MRENCLAVE bump, fresh DKG. Does NOT touch escrow funds, master-key state, or `SignerListSet` membership. Can run on an empty escrow, on a partially-funded escrow, or on a fully-provisioned escrow — the procedure is identical and indifferent.
