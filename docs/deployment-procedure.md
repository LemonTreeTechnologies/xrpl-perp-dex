...
> 2. **FROST 2-of-3 threshold-Schnorr is a separate, live facility** — an in-memory DKG group (`frost_groups[]` + `active_frost_group`), re-shared between nodes over libp2p on restart and migrated across an MRENCLAVE bump as **Path A section 2** (Path A was in fact built to transport this share — commit `a6bbbb6`). Its signing is for the Taproot/BTC layer (**roadmap** — not yet a shipped product path), and in no case for XRPL settlement (REQ-12 §3, pattern 5.42). It is **not latent** as a facility (DKG + libp2p share-transport run today) — it is simply a different layer from the on-chain XRPL quorum, with no product signing use live yet.
>
...
**Posture note (A-PA-1, REQ-18).** Path A LA-report verification rejects a debug-mode peer enclave when the importer is non-debug. A debug→non-debug migration is therefore rejected by design — the debug→production transition is a fresh bootstrap (`node-deploy` without `--side-by-side`), NOT a Path A migration (a debug enclave's exported state is not trustworthy). Path A applies only within one debug posture: debug→debug on testnet, non-debug→non-debug in production.
