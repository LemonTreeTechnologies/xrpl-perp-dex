...
2. **Ни один оператор не может произвести валидную FROST подпись.** FROST 2-of-N (или выше) предотвращает это; `frost_group` state sealed внутри каждого enclave'а. Это **отдельный механизм от №1, и не он защищает XRPL-кастоди** — сегодня ни один продуктовый путь perp-dex не подписывает через FROST; FROST-группа существует под планируемую Bitcoin/Taproot-ветку, а групповой ключ FROST не входит в XRPL SignerList.
...
6. **Signing-policy check orchestrator-anchored — Pattern 318 deviation acknowledged.** Orchestrator-anchored signing-policy check (X-C1 hardening per audit Round 4-6) живёт в коде которое контролирует operator host и SGX не attestирует. Compensating control — loopback constraint в трёх слоях: enclave `bind_address` self-check (perp REQ-6 C3), orchestrator `ensure_loopback_url` на `local_signer.enclave_url` (perp REQ-6 C2), OS firewall ACLs (out-of-band, см. `feedback_firewall_topology_neutrality.md`). Любое будущее изменение делающее `/pool/sign` non-loopback ИЛИ любой новый signing path обходящий `validate_signing_policy` invalidate'ит эту equivalence и требует свежий policy-anchor analysis (Pattern 318 deviation per audit playbook). Structurally-correct enclave-anchored alternative (`ecall_check_pending_signing`) отслеживается как RESP-5 §C5 follow-up, запланирован post-mainnet-thaw.
7. **Production-mode недостижим без reproducible MRENCLAVE produced независимо ≥N operators.** Foundation invariant установленный 2026-05-03 (peer of #5). Если только один оператор может произвести given MRENCLAVE, multi-operator trust model вырождается в "trust one operator's build environment" — supply-chain compromise того одного host'а равен compromise всей группы. Build не требует SGX hardware; prerequisite — committed reproducible Dockerfile (`EthSignerEnclave/Dockerfile.azure`) с pinned toolchain versions, GHA build-gate который publishes MRENCLAVE per commit, и N-of-M cross-check procedure где ≥N независимых operators rebuild и publish bit-identical MRENCLAVE values которые потом registered на on-chain MRENCLAVE allowlist (per REQ-7). Текущие sandbox modes (§1.1, §1.2 of `docs/development-operating-model.md`) допускают single-operator builds потому что не держат real customer state; production-mode (§1.3) gated на этом rule И на #5. См. `docs/build-requirements.{en,ru}.md` для layered Build / Sign / Run policy и `feedback_reproducible_build_foundation.md` для правила.
...
Phase 2.1c полностью выполнен и live-validated. Репозиторий больше не содержит SSH-driven cross-operator кодовых путей.

### 11.4 Phase 2.2 — governance (membership change)

§8 add/remove-operator раскладывается на уже отгруженные примитивы плюс один новый wedge: governance multisig через `SignerListSet`. Phase 2.2 добавляет его без ослабления X-C1 invariants которые существующий signing relay уже enforces.

| Phase | Surface | Класс | Status |
|---|---|---|---|
| 2.2-A | `validate_signing_policy` per-`TransactionType` dispatcher + `validate_signerlist_set_specific` (14 constraint'ов, 28 unit-тестов) | (в `p2p.rs`) | ✅ landed `fce46ff` |
| 2.2-B | wire-level integration tests против in-process axum mock enclave'а (4 `#[tokio::test]`) | (в `p2p.rs`) | ✅ landed `f7a707f` |
| 2.2-C | `signerlist_update.rs` admin route (`POST /admin/signerlist-update`) + `--signerlist-admin-listen` CLI flag (18 unit-тестов) | cluster-coordinated | ✅ landed `0b1dae5` |
| 2.2-D | live testnet rotation 2-of-3 ↔ 3-of-3, on-chain подтверждение | (live-only) | ✅ validated 2026-04-29 |

Relay теперь принимает два `TransactionType`'а — `Payment` (сохранён verbatim, audit-approved) и `SignerListSet`. У каждого tx type свой dedicated validator со своим constraint set'ом; универсальная X-C1 hardening (replay guard, peer allowlist, per-peer rate limit, source-account binding, multisig marker, signer-identity binding) живёт в dispatcher'е и применяется к каждому allowed type'у. Equal-weight (`SignerWeight=1`) и SignerQuorum∈[2, N] enforced pre-sign — quorum-math footgun'ы недостижимы.

Полный §8 add/remove-operator flow теперь code-complete: `node-bootstrap` (новый оператор) → `signerlist-update --add r…` (existing-quorum multisig через relay) → on-chain SignerListSet → `node-config-apply` (каждый оператор re-discovers roster) → `dkg-coordinate` (свежий group_pubkey под новым membership). Никакого SSH; никакой ручной XRPL-cli ceremony; никакого per-environment branching'а.
