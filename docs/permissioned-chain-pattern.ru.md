...
- **TEE-attested business logic** — perpetuals matching engine, vault, settlement math и пороговая подпись сеттлмента (XRPL SignerList M-of-N ECDSA — примитивы FROST/Schnorr в энклейве тоже есть, но на XRPL они ничего не подписывают). Runs in SGX сегодня; портабельно на TDX / SEV-SNP / Nitro в principle — см. `EthSignerEnclave/docs/contingency/` в private repo для substrate-migration анализов.
