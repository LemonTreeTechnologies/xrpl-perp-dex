...
pub enum SigningMessage {
    ...
    /// #131 AC-BASE-2″ P2-c SPV ceremony. The retired scalar request (#131 R-1) shipped a
    /// host FIGURE each receiver re-queried; this ships ONE validator-quorum-attested proof
...
/// Per-node cosigner config: the LOCAL escrow this node holds custody for, and this
/// node's OWN XRPL endpoint. `None` on a node = it never cosigns (fail-closed).
///
/// #131 R-1: this was the scalar baseline receiver's config; that ceremony is retired.
/// What still needs it is the §6 UNL-policy cosigner, which reads its OWN validated
/// ledger through `xrpl_endpoint` rather than taking the leader's proposed anchor on
/// trust (Q-UNL-4). The issuer and shard_id fields went with the retired hash.
#[derive(Debug, Clone)]
pub struct NodeCosignerConfig {
    /// Escrow classic r-address this node's enclave holds custody for (context for logs).
    pub escrow_r: String,
...
pub struct P2PNode {
    ...
    reserves_baseline_config: Option<NodeCosignerConfig>,
...
impl P2PNode {
    ...
    /// Install this node's cosigner config (local escrow + its OWN XRPL endpoint).
    /// Without it, this node never cosigns (fail-closed).
    #[allow(dead_code)]
    pub fn set_cosigner_config(&mut self, cfg: NodeCosignerConfig) {
        info!(
            escrow = %cfg.escrow_r,
            endpoint = %cfg.xrpl_endpoint,
            "P2P: cosigner config set (own XRPL endpoint for the §6 UNL-policy check)"
        );
